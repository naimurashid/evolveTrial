...
cum_person_time_all_arms <- function(state, current_time, max_follow_up, interval_cutpoints, arm_names) {
  sum(vapply(arm_names, function(a) {
    cum_person_time_arm(state, a, current_time, max_follow_up, interval_cutpoints)
  }, numeric(1)))
}
...
calculate_weibull_scale <- function(desired_median, weibull_shape) {
  stopifnot(
    is.numeric(desired_median), is.numeric(weibull_shape),
...
draw_posterior_hazard_samples <- function(
  ...
  if (!all(sapply(
    list(events_per_interval, person_time_per_interval, prior_alpha_params, prior_beta_params),
    length
  ) == num_intervals)) {
...
calculate_predicted_prob_vs_ref <- function(
  ...
  # NEW: speed/early-exit knobs (optional)
  inner_final_draws = 250,
  target_pred_efficacy_threshold = NULL, # e.g., your pred_success_pp_threshold_vs_ref
  target_pred_futility_threshold = NULL # e.g., your pred_futility_pp_threshold_vs_ref
) {
  # Setup
  num_intervals <- length(interval_cutpoints) - 1
  interval_lengths <- diff(interval_cutpoints)
  ...
  for (k in 1:num_posterior_draws) {
    ...
    futA <- if (nA > 0) {
      simulate_piecewise_exponential_data(
        nA, lamA, interval_cutpoints, max_follow_up_sim, censor_max_time_sim,
    ...
    futR <- if (nR > 0) {
      simulate_piecewise_exponential_data(
        nR, lamR, interval_cutpoints, max_follow_up_sim, censor_max_time_sim,
    ...
    p_fut_final <- mean(medA < medR * compare_arms_futility_margin)

    # Indicators: would FINAL posterior declare efficacy/futility?
    eff_k <- as.integer(p_eff_final >= final_efficacy_posterior_prob_threshold)
    fut_k <- as.integer(p_fut_final >= final_futility_posterior_prob_threshold)

    s_eff <- s_eff + eff_k
    s_fut <- s_fut + fut_k

    # --- Early exit on bounds if targets provided ---
    n_rem <- N - k
    if (!is.null(target_pred_efficacy_threshold)) {
      ub_eff <- (s_eff + n_rem) / N
      lb_eff <- s_eff / N
      if (ub_eff < target_pred_efficacy_threshold || lb_eff > target_pred_efficacy_threshold) {
        # We cannot change the decision wrt the efficacy target; we may still need futility
        # but in practice you usually call once per look; break if both targets are decided.
        if (is.null(target_pred_futility_threshold)) break
      }
    }
    if (!is.null(target_pred_futility_threshold)) {
      ub_fut <- (s_fut + n_rem) / N
      lb_fut <- s_fut / N
    ...
    # If both targets provided and both already decided by bounds, break:
...
slice_arm_data_at_time <- function(registry_df, calendar_time, max_follow_up, interval_cutpoints) {
  ...
  # Use the FAST metrics consistently
...
interim_check <- function(state, current_time, args, diagnostics = FALSE) {
  get_arg <- function(nm, default) if (!is.null(args[[nm]])) args[[nm]] else default

  pred_eff_hc_thr <- get_arg("pred_success_pp_threshold_hc", 0.90)
  pred_fut_hc_thr <- get_arg("pred_futility_pp_threshold_hc", 0.10)
  pred_eff_ref_thr <- get_arg("pred_success_pp_threshold_vs_ref", 0.90)
  pred_fut_ref_thr <- get_arg("pred_futility_pp_threshold_vs_ref", 0.90)

  final_succ_thr <- get_arg(
    "final_success_posterior_prob_threshold",
    get_arg("efficacy_threshold_current_prob_hc", 0.90)
  )
  final_fut_thr <- get_arg(
    "final_futility_posterior_prob_threshold",
    get_arg("posterior_futility_threshold_hc", 0.90)
  )

  maxN_per_arm <- args$max_total_patients_per_arm
  interval_lengths <- diff(args$interval_cutpoints_sim)
  num_intervals <- length(interval_lengths)

  predictive_fast <- isTRUE(get_arg("predictive_fast", TRUE))
  num_draws_pred <- as.integer(get_arg("num_posterior_draws_pred", 400))
  boundary_band <- as.numeric(get_arg("predictive_boundary_band", 0.12))
  run_pred_every_k_beats <- as.integer(get_arg("run_pred_every_k_beats", 1L))
  beat_index <- floor(current_time / get_arg("interim_calendar_beat", 1))
  allow_pred_this_look <- (beat_index %% run_pred_every_k_beats) == 0

  near_boundary <- function(prob_now, target, band = boundary_band) {
    is.finite(prob_now) && abs(prob_now - target) <= band
  }

  # Per-arm gate settings
  min_ev_arm <- get_arg("min_events_per_arm", get_arg("min_events_for_analysis", 0))
  min_fu_arm <- get_arg("min_median_followup_per_arm", get_arg("min_median_followup", 0))
  min_pt_frac <- get_arg("min_person_time_frac_per_arm", 0)
  ev_ratio_min <- get_arg("min_events_ratio_arm_vs_ref", 0.0)

  # Max PT (for per-arm PT fraction gate)
  max_PT_per_arm <- args$max_PT_per_arm %||% setNames(
    as.numeric(args$max_total_patients_per_arm) * args$max_follow_up_sim,
    names(args$max_total_patients_per_arm)
  )

  # Reference posterior medians if analyzable
  build_ref_if_ready <- function() {
    ref <- args$reference_arm_name
    if (!args$compare_arms_option || state$arm_status[ref] != "recruiting") {
      return(NULL)
    }
    if (state$enrolled_counts[ref] < args$min_patients_for_analysis) {
      return(NULL)
    }

    ref_slice <- slice_arm_data_at_time(
      state$registries[[ref]], current_time,
      args$max_follow_up_sim, args$interval_cutpoints_sim
    )
    if (sum(ref_slice$patient_data$event_status) < get_arg("min_events_for_analysis", 0)) {
      return(NULL)
    }
    if (nrow(ref_slice$patient_data) == 0 ||
      median(ref_slice$patient_data$observed_time) < get_arg("min_median_followup", 0)) {
      return(NULL)
    }

    ref_post <- draw_posterior_hazard_samples(
      num_intervals,
      ref_slice$metrics$events_per_interval,
      ref_slice$metrics$person_time_per_interval,
      args$prior_alpha_params_model,
      args$prior_beta_params_model,
      num_samples = min(args$num_posterior_draws, 1500)
    )
    apply(ref_post, 1, function(h) calculate_median_survival_piecewise(h, interval_lengths))
  }
  med_ref <- build_ref_if_ready()

  for (arm in args$arm_names) {
    if (state$arm_status[arm] != "recruiting") next
    if (state$enrolled_counts[arm] < args$min_patients_for_analysis) next

    arm_slice <- slice_arm_data_at_time(
      state$registries[[arm]], current_time,
      args$max_follow_up_sim, args$interval_cutpoints_sim
    )

    # PER-ARM GATES (use metrics for PT)
    ev_arm <- sum(arm_slice$patient_data$event_status)
    medFU <- if (nrow(arm_slice$patient_data) > 0) median(arm_slice$patient_data$observed_time) else 0
    pt_arm <- sum(arm_slice$metrics$person_time_per_interval)
    pt_req <- max_PT_per_arm[arm] * min_pt_frac
    if (ev_arm < min_ev_arm || medFU < min_fu_arm || pt_arm < pt_req) next

    # Symmetric gates for ref (if comparing)
    if (args$compare_arms_option && arm != args$reference_arm_name) {
      ref <- args$reference_arm_name
      ref_slice <- slice_arm_data_at_time(
        state$registries[[ref]], current_time,
        args$max_follow_up_sim, args$interval_cutpoints_sim
      )
      ev_ref <- sum(ref_slice$patient_data$event_status)
      medFU_r <- if (nrow(ref_slice$patient_data) > 0) median(ref_slice$patient_data$observed_time) else 0
      pt_ref <- sum(ref_slice$metrics$person_time_per_interval)
      pt_req_r <- max_PT_per_arm[ref] * min_pt_frac

      if (ev_ref < min_ev_arm || medFU_r < min_fu_arm || pt_ref < pt_req_r) next
      if (ev_ratio_min > 0 && ev_arm < ev_ratio_min * ev_ref) next
    }

    decided <- FALSE

    # 1) FAST PREDICTIVE
    if (predictive_fast && allow_pred_this_look) {
      if (!args$compare_arms_option) {
        post_arm_quick <- draw_posterior_hazard_samples(
          num_intervals,
          arm_slice$metrics$events_per_interval,
          arm_slice$metrics$person_time_per_interval,
          args$prior_alpha_params_model,
          args$prior_beta_params_model,
          num_samples = 200
        )
        med_arm_quick <- apply(post_arm_quick, 1, function(h) {
          calculate_median_survival_piecewise(h, interval_lengths)
        })
        pr_eff_hc_now <- mean(med_arm_quick > args$null_median_arms[arm])
        pr_fut_hc_now <- mean(med_arm_quick < args$futility_median_arms[arm])

        run_pred_now <- near_boundary(pr_eff_hc_now, get_arg("efficacy_threshold_current_prob_hc", 0.90)) ||
          near_boundary(pr_fut_hc_now, get_arg("posterior_futility_threshold_hc", 0.90))

        if (run_pred_now) {
          pps_prob <- tryCatch(
            calculate_predicted_success_prob_vs_hc_fast(
              current_patient_data = arm_slice$patient_data,
              current_enroll_times = state$registries[[arm]]$enroll_time %||% numeric(0),
              max_total_patients = maxN_per_arm[arm],
              ...
              num_posterior_draws = num_draws_pred,
              median_pfs_success_threshold = get_arg("median_pfs_success_threshold_arms", args$null_median_arms)[arm],
              final_success_posterior_prob_threshold = final_succ_thr,
              max_follow_up_sim = args$max_follow_up_sim,
              overall_accrual_rate = args$overall_accrual_rate,
              current_time = current_time
            ),
            error = function(e) NA_real_
          )

          if (!is.na(pps_prob) && (pps_prob >= pred_eff_hc_thr)) {
            state$stop_efficacy_per_sim_row[arm] <- 1L
            state$arm_status[arm] <- "stopped_efficacy_pred_fast"
            state$sim_final_n_current_run[arm] <- state$enrolled_counts[arm]
            decided <- TRUE
          } else if (!is.na(pps_prob) && (pps_prob <= pred_fut_hc_thr)) {
            state$stop_futility_per_sim_row[arm] <- 1L
            state$arm_status[arm] <- "stopped_futility_pred_fast"
            state$sim_final_n_current_run[arm] <- state$enrolled_counts[arm]
            decided <- TRUE
          }
        }
      } else if (arm != args$reference_arm_name && !is.null(med_ref)) {
        ref <- args$reference_arm_name
        ref_slice <- slice_arm_data_at_time(
          state$registries[[ref]], current_time,
          args$max_follow_up_sim, args$interval_cutpoints_sim
        ...
        pred_ref <- tryCatch(
          calculate_predicted_prob_vs_ref_fast(
            current_patient_data_arm = arm_slice$patient_data,
            current_patient_data_ref = ref_slice$patient_data,
            enroll_times_arm = state$registries[[arm]]$enroll_time %||% numeric(0),
            enroll_times_ref = state$registries[[ref]]$enroll_time %||% numeric(0),
            max_total_patients_arm = maxN_per_arm[arm],
            max_total_patients_ref = maxN_per_arm[ref],
            interval_cutpoints = args$interval_cutpoints_sim,
            prior_alpha_params = args$prior_alpha_params_model,
            prior_beta_params = args$prior_beta_params_model,
            num_posterior_draws = num_draws_pred,
            final_efficacy_posterior_prob_threshold = get_arg(
              "final_efficacy_posterior_prob_threshold",
              get_arg("efficacy_threshold_vs_ref_prob", 0.90)
            ),
            final_futility_posterior_prob_threshold = get_arg(
              "final_futility_posterior_prob_threshold",
              get_arg("futility_threshold_vs_ref_prob", 0.90)
            ),
            compare_arms_futility_margin = args$compare_arms_futility_margin,
            max_follow_up_sim = args$max_follow_up_sim,
            overall_accrual_rate = args$overall_accrual_rate,
            current_time = current_time
          ),
        ...
        if (!is.null(pred_ref)) {
          if (pred_ref$predicted_prob_efficacy >= pred_eff_ref_thr) {
            state$stop_efficacy_per_sim_row[arm] <- 1L
            state$arm_status[arm] <- "stopped_efficacy_pred_ref_fast"
            state$sim_final_n_current_run[arm] <- state$enrolled_counts[arm]
            decided <- TRUE
          } else if (pred_ref$predicted_prob_futility >= pred_fut_ref_thr) {
            state$stop_futility_per_sim_row[arm] <- 1L
            state$arm_status[arm] <- "stopped_futility_pred_ref_fast"
            state$sim_final_n_current_run[arm] <- state$enrolled_counts[arm]
            decided <- TRUE
          }
        }
    ...
    if (diagnostics) {
      n_arm <- nrow(arm_slice$patient_data)
      ev_arm2 <- sum(arm_slice$patient_data$event_status)
      med_fu <- if (n_arm > 0) median(arm_slice$patient_data$observed_time) else NA_real_
      mean_fu <- if (n_arm > 0) mean(arm_slice$patient_data$observed_time) else NA_real_
      if (!is.null(med_ref) && arm != args$reference_arm_name) {
        pr_eff_ref <- mean(med_arm > med_ref)
        pr_fut_ref <- mean(med_arm < med_ref * args$compare_arms_futility_margin)
        cat(sprintf(
          "[t=%.2f] %s: N=%d ev=%d medFU=%.2f meanFU=%.2f | vsREF PrEff>=%.2f:%.3f PrFut>=%.2f (m=%.2f):%.3f\n",
          current_time, arm, n_arm, ev_arm2, med_fu, mean_fu,
          args$efficacy_threshold_vs_ref_prob, pr_eff_ref,
          args$futility_threshold_vs_ref_prob, args$compare_arms_futility_margin, pr_fut_ref
        ))
      } else {
        pr_eff_hc <- mean(med_arm > args$null_median_arms[arm])
        pr_fut_hc <- mean(med_arm < args$futility_median_arms[arm])
        cat(sprintf(
          "[t=%.2f] %s: N=%d ev=%d medFU=%.2f meanFU=%.2f | vsHC  PrEff>=%.2f:%.3f PrFut>=%.2f:%.3f\n",
          current_time, arm, n_arm, ev_arm2, med_fu, mean_fu,
          args$efficacy_threshold_current_prob_hc, pr_eff_hc,
          args$posterior_futility_threshold_hc, pr_fut_hc
        ))
      }
    }

    if (args$compare_arms_option && arm != args$reference_arm_name && !is.null(med_ref)) {
      pr_eff_ref <- mean(med_arm > med_ref)
      pr_fut_ref <- mean(med_arm < med_ref * args$compare_arms_futility_margin)

      if (pr_eff_ref >= args$efficacy_threshold_vs_ref_prob) {
        state$stop_efficacy_per_sim_row[arm] <- 1L
        state$arm_status[arm] <- "stopped_efficacy_ref"
        state$sim_final_n_current_run[arm] <- state$enrolled_counts[arm]
      } else if (pr_fut_ref >= args$futility_threshold_vs_ref_prob) {
        state$stop_futility_per_sim_row[arm] <- 1L
        state$arm_status[arm] <- "stopped_futility_ref"
        state$sim_final_n_current_run[arm] <- state$enrolled_counts[arm]
      }
    } else {
      pr_eff_hc <- mean(med_arm > args$null_median_arms[arm])
      ...
      if (pr_eff_hc >= args$efficacy_threshold_current_prob_hc) {
        state$stop_efficacy_per_sim_row[arm] <- 1L
        state$arm_status[arm] <- "stopped_efficacy_hc"
        state$sim_final_n_current_run[arm] <- state$enrolled_counts[arm]
      } else if (pr_fut_hc >= args$posterior_futility_threshold_hc) {
        state$stop_futility_per_sim_row[arm] <- 1L
        state$arm_status[arm] <- "stopped_futility_hc"
        state$sim_final_n_current_run[arm] <- state$enrolled_counts[arm]
      }
    }
  } # arm loop
...
run_simulation_pure <- function(
  ...
  final_n_per_sim <- matrix(0,
    nrow = num_simulations, ncol = length(arm_names),
    dimnames = list(NULL, arm_names)
  )
  stop_efficacy_per_sim <- matrix(0,
    nrow = num_simulations, ncol = length(arm_names),
    dimnames = list(NULL, arm_names)
  )
  stop_futility_per_sim <- matrix(0,
    nrow = num_simulations, ncol = length(arm_names),
    dimnames = list(NULL, arm_names)
  )
  final_efficacy_per_sim <- matrix(0,
    nrow = num_simulations, ncol = length(arm_names),
    dimnames = list(NULL, arm_names)
  )
  final_futility_per_sim <- matrix(0,
    nrow = num_simulations, ncol = length(arm_names),
    dimnames = list(NULL, arm_names)
  )
  final_inconclusive_per_sim <- matrix(0,
    nrow = num_simulations, ncol = length(arm_names),
    dimnames = list(NULL, arm_names)
  )
  ...
  max_PT_per_arm <- setNames(
    as.numeric(max_total_patients_per_arm) * max_follow_up_sim,
  ...
  for (s in 1:num_simulations) {
    ...
    while (length(is_eligible(state)) > 0) {
      ...
      if (!is.null(pt_targets_abs)) {
        ...
        total_PT_now <- cum_person_time_all_arms(
          state, current_time, max_follow_up_sim,
      ...
      t_event_true <- rweibull(1,
        shape = weibull_shape_true_arms[chosen_arm],
        scale = weibull_scale_true_arms[chosen_arm]
      )
      t_random_censor <- runif(1, min = 0, max = censor_max_time_sim)

      state$registries[[chosen_arm]] <- rbind(
        state$registries[[chosen_arm]],
        data.frame(
          id = patient_id,
    ...
    for (arm in arm_names) {
      ...
      if (!compare_arms_option) {
        ...
        if (arm == reference_arm_name) {
          ...
          p_eff_ref <- mean(med_arm > med_ref)
          p_fut_ref <- mean(med_arm < med_ref * compare_arms_futility_margin)
...
pretty_scenario_matrix <- function(results_df) {
  ...
  wide <- data.table::dcast(
    ...
    scenario ~ Arm_Name,
    value.var = c(
      "True_Median", "Exp_N", "Pr_Reach_Max_N",
...
calibrate_alpha <- function(base_args, scens_null, thr_grid_interim = c(0.9, 0.95, 0.975),
  ...
  for (ti in thr_grid_interim) {
    for (tf in thr_grid_final) {
      args <- base_args
...
grid_calibrate <- function(base_args,
  ...
  list(
    all = grid[order(margin_abs, interim_thr, final_thr)],
    feasible = ok,
    top = head(ok, 10)
  )
}
...
calculate_predicted_prob_vs_ref_fast <- function(
  ...
  T_future_a <- approx_future_exposure(
    current_time, enroll_times_arm,
    max_total_patients_arm, overall_accrual_rate,
    interval_cutpoints, max_follow_up_sim
  )
  T_future_r <- approx_future_exposure(
    current_time, enroll_times_ref,
  ...
  for (k in seq_len(num_posterior_draws)) {
    ...
    alpha_af <- alpha_a0 + ev_a
    beta_af <- beta_a0 + T_future_a
    alpha_rf <- alpha_r0 + ev_r
    beta_rf <- beta_r0 + T_future_r

    lam_af <- rgamma(K, shape = alpha_af, rate = beta_af)
    lam_rf <- rgamma(K, shape = alpha_rf, rate = beta_rf)

    med_a <- calculate_median_survival_piecewise(lam_af, interval_lengths)
    med_r <- calculate_median_survival_piecewise(lam_rf, interval_lengths)

    eff_hits <- eff_hits + as.integer(med_a > med_r)
    fut_hits <- fut_hits + as.integer(med_a < med_r * compare_arms_futility_margin)
  }
